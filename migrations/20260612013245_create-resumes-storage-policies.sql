...
CREATE POLICY storage_objects_resumes_select
ON storage.objects
FOR SELECT
TO authenticated
USING (
  ...
  AND (
    ...
    OR (
      (storage.foldername(key))[1] = 'resumes'
      AND (storage.foldername(key))[2] = (SELECT auth.jwt() ->> 'sub')
    )
  )
);

CREATE POLICY storage_objects_resumes_insert
ON storage.objects
FOR INSERT
TO authenticated
WITH CHECK (
...
CREATE POLICY storage_objects_resumes_update
ON storage.objects
FOR UPDATE
TO authenticated
USING (
...
CREATE POLICY storage_objects_resumes_delete
ON storage.objects
FOR DELETE
TO authenticated
USING (
